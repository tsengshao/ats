...
def cal_CWVatDongsha(data_wrapper):
    ...
    for i in range(len(level)):
        q[:,:,i] = data_wrapper(var="q",lev=level[i],time=0)
    # calculation
...
def cal_WSatBanqiao(data_wrapper):
    ...
    for i in range(len(level)):
        u[:,:,i] = data_wrapper(var="u",lev=level[i],time=0)
        v[:,:,i] = data_wrapper(var="v",lev=level[i],time=0)
    # calculation
...
def cal_CrossStraitUatNWTW(data_wrapper):
    ##### R10229007 #####
    # load
    u = data_wrapper(var="u",lev=700,time=0)
    v = data_wrapper(var="v",lev=700,time=0)
    # area
...
def cal_UatNETW(data_wrapper):
    ##### R10229007 #####
    # load
    u = data_wrapper(var="u",lev=950,time=0)
    v = data_wrapper(var="v",lev=950,time=0)
    # area
...
def cal_swDEPTHatNETW(data_wrapper):
    ...
    for i in range(len(level)):
        u[:,:,i] = data_wrapper(var="u",lev=level[i],time=0)
        v[:,:,i] = data_wrapper(var="v",lev=level[i],time=0)
        z[:,:,i] = data_wrapper(var="z",lev=level[i],time=0)/g
    # area
    uatNETW = np.mean(u[4:9,24:29,:],axis=(0,1))
    vatNETW = np.mean(v[4:9,24:29,:],axis=(0,1))
...
def cal_MSEatNETW(data_wrapper):
    ...
    q = np.zeros([lats,lons,len(level)])
    for i in range(len(level)):
        T[:,:,i] = data_wrapper(var="T",lev=level[i],time=0)
        z[:,:,i] = data_wrapper(var="z",lev=level[i],time=0)
        q[:,:,i] = data_wrapper(var="q",lev=level[i],time=0)
    # calculation
    mse = Cp*T+z+Lv*q
    # area
...
def cal_LeeVortex(data_wrapper):
    ##### R10229007 #####
    # load
    zeta = data_wrapper(var="vor",lev=950,time=0)
    # area
...
def cal_CAPEatNETW(data_wrapper):
    ##### B07209039 #####
    # parameters
    A = 2.53*10**11 # [Pa]
    B = 5420 # [K]
    ...
    Td = -B/(np.log(e/A))
    # area
    TatNETW = np.mean(T[4:7,24:27,:],axis=(0,1))
    TdatNETW = np.mean(Td[4:7,24:27,:],axis=(0,1))
    # calculation
    CAPEatNETW,CINatNETW = mpcalc.surface_based_cape_cin(level*units.hPa,TatNETW*units.kelvin,TdatNETW*units.kelvin)
    return CAPEatNETW.m
...
def cal_deltaLTSatSWTW(data_wrapper):
    ...
    lats = 41
    # load
    theta700 = data_wrapper(var="T",lev=700,time=0)*(1000/700)**(Rd/Cp)
    theta1000 = data_wrapper(var="T",lev=1000,time=0)
    # calculation
