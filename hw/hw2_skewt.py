...
def plot_skewt_mse(
    ...
    *,
    ...
    # base profiles
    ax_skew.semilogy(T, p, 'r', lw=4.0, label='T')
    ax_skew.semilogy(Td, p, 'g', lw=4.0, label='Td')
    ax_skew.axvline(0, color='b', lw=1.0)
    ax_skew.semilogy(parcel_t - 273.15, parcel_lev, 'k', lw=4.0, label='Parcel')
    ...
    ax_mse.plot(the, p, 'tab:blue', lw=4.0, label=r'$\theta_e$')
    ax_mse.plot(thes, p, 'tab:red', lw=4.0, label=r'$\theta_{es}$')
    ax_mse.plot(parcel_thes, parcel_lev, 'k', lw=4.0, label=r'Parcel $\theta_{es}$')
...
def read_era(nowtime, lonp, latp, lev_lowb=[1000, 100]):
    lonb = [lonp,lonp+0.001]
    latb = [latp,latp+0.001]
    lon_e,  lat_e,  lev_e, q_e   = uread.read_era5_3d('q',nowtime,lonb,latb,lev_lowb)
    _, _, _, u_e   = uread.read_era5_3d('u',nowtime,lonb,latb,lev_lowb)
    _, _, _, v_e   = uread.read_era5_3d('v',nowtime,lonb,latb,lev_lowb)
    _, _, _, t_e   = uread.read_era5_3d('t',nowtime,lonb,latb,lev_lowb)
    return lon_e, lat_e, lev_e, np.squeeze(t_e), np.squeeze(q_e), np.squeeze(u_e), np.squeeze(v_e)
