...
def draw_ivt_and_local_rain(nowtime, only_output_flag=False):
    ...
    # lon_0p1_e, lat_0p1_e, suf_geo_e  = uread.read_era5_2d('suf_geo', nowtime,lonb,latb)
    ...
    # figure 1, observation rain gauge
    ...
    CO    = ax1.contour(ds_topo.lon, ds_topo.lat, 
    ...
    IM    = ax1.scatter(stn_lon, stn_lat, c=stn_rain, s=100, 
                 cmap=rain_cmap, norm=rain_norm, alpha=0.7, edgecolor='none')
    ax1.axis((119.8, 122.2, 21.8, 25.6))

    cbar = fig.colorbar(IM, orientation='vertical', cax=cax1, extend='max')
    cbar.ax.set_title('rain[mm/d]', fontsize=15, y=1.05, loc='left')
    cbar = fig.colorbar(CO, orientation='vertical', cax=cax2)
    ...
    ax1.set_title(f'accumulated rainfall',
        loc='right', fontweight='bold', fontsize=15)
    wtype_str = decide_weather(sw_ivt_mean, sw_ivt_dir, tc_ivt_max, tc_ivt_zeta_max)
    ...
    datestr=nowtime.strftime("%Y%m%d")
    plt.savefig(f'./hw2_cwarain_{datestr}_{wtype_str}_{distr}.png', facecolor='w', bbox_inches='tight', dpi=400)

    # raingauge timeseries
    fig, ax3 = plt.subplots(figsize=(12,5))
    # subplots, ax3, timeseries
    x = np.arange(1,25)
    nstn = stn_rain_hr.shape[0]
    for istn in range(nstn):
        ax3.plot(x, stn_rain_hr[istn], color='0.5', lw=0.5)
    ax3.plot(x, np.nanmean(stn_rain_hr,axis=0), color='#D70801')
    ax3.set_xticks(np.arange(1,25,1))
    ...
    ax3.set_title(f'{datestr}\nCWA rain gauge',
        loc='left', fontweight='bold', fontsize=20)
    ax3.set_title(f'mean ({nstn})',
        loc='right', fontweight='bold', fontsize=20)
    plt.grid()
    plt.savefig(f'./hw2_rainseries_{datestr}_{wtype_str}_{distr}.png', facecolor='w', bbox_inches='tight', dpi=400)
    ...
    # # ERA5 TOPO
    # CO=ax1.contourf(lon_0p1_e, lat_0p1_e, np.where(topo_e>50,topo_e,np.nan),
    ...
    ## # IVT strength
    ## dum = np.sqrt(ivtx_e**2+ivty_e**2)
    ## PC = ax1.pcolormesh(lon_e, lat_e, np.where(dum>=ivt_bounds[0],dum,np.nan),
    ##                     norm=ivt_norm,cmap=ivt_cmap)
    ...
    ## datestr=nowtime.strftime("%Y%m%d")
    ## plt.savefig(f'./hw1_era5_ivt_{datestr}_{wtype_str}_{distr}.png', facecolor='w', bbox_inches='tight', dpi=400)
    return output
