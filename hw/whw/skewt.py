...
if __name__=="__main__":
    ...
    p_ori, T_ori, q_ori = np.loadtxt('IdealizedSounding.csv',usecols=[0,2,4], unpack=True, skiprows=1, delimiter=',')

    p = np.arange(1000, 100-0.01, -1)
    T = np.interp(np.log(p[::-1]), np.log(p_ori[::-1]), T_ori[::-1])[::-1]
    q = np.interp(np.log(p[::-1]), np.log(p_ori[::-1]), q_ori[::-1])[::-1]

    u = np.zeros(p.size)
    v = np.zeros(p.size)

   # P(hPa),H(m),T(K),Td(K),qv(g/kg)
   # 1000.000,115.288,296.726,292.556,14.439
   # 975.000,336.469,294.674,291.904,14.175
    ...
    ###     idx0 = np.argmin(np.abs(pres-1000.))
    ###     idx1 = np.argmin(np.abs(pres-500.))
    ###     Tnew = temp.copy()*np.nan
    ###     Qnew = qv.copy()*np.nan
    ###     Tdnew = Td.copy()*np.nan
    ###     for i in range(idx0, idx1):
    ###         print(i,idx0,idx1)
    ###         p_lev, p_t, p_qv, p_thes, lev_idx = parcel_profile(pres[i], temp[i], qv[i])
    ###         idxnew = np.argmin(np.abs(hei-(hei[i]+1000.)))
    ###         idxp   = np.argmin(np.abs(p_lev-pres[idxnew]))
    ###         Tnew[idxnew] = p_t[idxp]
    ###         Qnew[idxnew] = p_qv[idxp]
    ###         Tdnew[idxnew] = cal_dewpoint_from_p_T_q(p_lev[idxp], Tnew[idxnew], Qnew[idxnew])

    ###     temp[:idxnew] = Tnew[:idxnew]
    ###     qv[:idxnew]   = Qnew[:idxnew]
    ###     #Td[:idxnew]   = Tdnew[:idxnew]
    ###     T = temp-273.15
    ###     pres       = p #hPa !!
    ...
    ###     the        = cal_equivalent_potential_temperature(pres,qv,temp)
    ###     thes       = cal_equivalent_potential_temperature(pres,qvs,temp)
    ###     idx00 = np.where(np.isnan(temp))[0].max()+1
    ###     parcel_lev, parcel_t, parcel_qv, parcel_thes, lev_idx = parcel_profile(pres[idx00], temp[idx00], qv[idx00])
    ...
    if not EL: EL=-999.9
    if not ELp_tv: ELp_tv=-999.9
    if not LFC: LFC=-999.9
    if not LFCp_tv: LFCp_tv=-999.9
